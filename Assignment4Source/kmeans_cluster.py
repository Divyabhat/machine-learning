...
def load_data(filename):
    print (filename)
    unprocessed_data_file = open('./optdigits/' + filename, 'r')
...
def entropy(cluster, labels):
    ...
    print('class representation',class_representation_in_cluster)
...
def k_means(testing_features, testing_labels, training_features,
    ...
    for trial in range(number_of_trials):
        ...
        sse = mean_squared_error(clustering, centers, training_features)
        sss = mean_squared_separation(clustering, centers)
        avg_entropy = mean_entropy(clustering, training_labels)

        k_means_trials[trial] = [centers, closest_centers, clustering, sse,
                                 sss, avg_entropy]

    # Choose the run (out of 5) that yields the smallest mean-squared error
    # - For this best run, in your report give the mean-squared error,
    # - mean-squared separation, and mean entropy of the resulting clustering.
...
def main():
    training_features, training_labels = load_data('optdigits.train')
