##Running Instruction --> python3 kmeans_exp.py
## kmeans algorithm  
...
class K_Means(object):
    ...
    def kmeans_train(self, k =10,max_val =16,cent_flag = 'randm'):
        ...
        if cent_flag == 'randm':
           self.centroids = np.random.randint(max_val+1, size=(self.k, self.n_feat))
            ...
            self.centroids = np.delete(self.train_data[ind], -1, 1)
    ...
    def kmeans_test(self):
        ...
        for i in range(self.k):
            sam_matrix[:, i, :] = np.subtract(self.centroids[i,:], np.delete(self.test_data, -1, 1))
        ...
        confusion_matrix = self.confu_matrix(clust_label,clust_val)
    ...
    # Create a confusion matrix
    def confu_matrix(self,clust_label,clust_val):
    ...
    def find_clusters(self, max_itr=300):
        ...
        for n in range(max_itr): 
            for i in range(self.k):
                sample_matrix[:, i, :] = np.subtract(self.centroids[i,:], np.delete(self.train_data, -1, 1))
            ...
            #temp_train_data = np.delete(self.train_data, -1, 1)
            clusters = [self.train_data[min_dist==cent] for cent in range(self.k)]

            for i in range(self.k):
                if len(clusters[i] > 0):
                    temp = np.mean(np.delete(clusters[i], -1, 1), axis=0)
    ...
    def calculate_avg_mse(self,clusters):
        ...
        for c in range(self.k):
            cluster = np.delete(clusters[c], -1, 1)
            mat = np.subtract(self.centroids[c], cluster)
    ...
    def calculate_entropy(self,clusters,k,datasize):
        ...
        classes = []; entropy = 0; coeff = []; clusterSize = []; countClass = 0
        for index in range(0,len(clusters)):
            ...
            clusterTotal = self.count_y(uniqueItems)
            log = 0
...
if __name__ == '__main__':
    ...
    print("Mean-entropy: " + str(kmeans_cluster_10.mean_entropy)) 
    print("Accuracy: " + str(accuracy))
    print("Confusion Matrix: ")
    print(confusion_matrix)

    ##FOR EXP 2, NUMBER OF CLUSTERS = 30
    ...
    print("Mean-entropy: " + str(kmeans_cluster_30.mean_entropy))
    print("Accuracy: " + str(accuracy))
    print("Confusion Matrix: ")
    print(confusion_matrix)
