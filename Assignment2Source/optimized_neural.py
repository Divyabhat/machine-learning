...
def plot_graph(hiddenNodes):
    trainFile = 'train_output' + str(hiddenNodes) + '.csv'
    validationFile = 'validation_output' + str(hiddenNodes) + '.csv'
    ...
    plt.legend()
    plt.savefig('graph' + str(hiddenNodes) + '.png')
    plt.cla()
...
class MultiLayerDigitIdentifier:
    ...
    def learn_and_find_accuracy(self, epoch, dataArray, targetClassList, trainingExecution):
        ...
        for i in range(0, dataArray.shape[0]):
            ...
            if trainingExecution and epoch > 0:
                ...
                deltaHiddenWeight = (self.learningRate * errorOutputLayer * self.hiddenWithBias.T) + (0.9 * self.wPreviousHiddenToOutput)
                self.wPreviousHiddenToOutput = deltaHiddenWeight
                self.wHiddenToOutput = self.wHiddenToOutput + deltaHiddenWeight

                deltaInputWeight = (self.learningRate * errorHiddenLayer * inputArray.T) + (0.9 * self.wPreviousInputToHidden)
    ...
    def execute_trained_data(self, epoch):
        ...
        self.store_accuracy('train_output' + str(self.hiddenNodes) + '.csv', epoch, accuracy)
    ...
    def execute_validation_data(self, epoch):
        ...
        self.store_accuracy('validation_output' + str(self.hiddenNodes) + '.csv', epoch, accuracy)
