...
trainingDataFile = "./mnist_train_half.csv"
...
def plot_graph(hiddenNodes):
    trainFile = 'train_output_exp2_half' + str(hiddenNodes) + '.csv'
    validationFile = 'validation_output_exp2_half' + str(hiddenNodes) + '.csv'
    ...
    plt.legend()
    plt.savefig('graph_exp2_half' + str(hiddenNodes) + '.png')
    plt.cla()
...
class MultiLayerDigitIdentifier:
    ...
    def execute_trained_data(self, epoch):
        ...
        self.store_accuracy('train_output_exp2_half' + str(self.hiddenNodes) + '.csv', epoch, accuracy)
    ...
    def execute_validation_data(self, epoch):
        ...
        self.store_accuracy('validation_output_exp2_half' + str(self.hiddenNodes) + '.csv', epoch, accuracy)
...
if __name__ == "__main__":
    ...
    halfList = list2[1::2]
    shuffledList = random.sample(halfList, len(halfList))


     ##writing 30000 training data into csv file
    my_file = pd.DataFrame(shuffledList)
    my_file.to_csv('mnist_train_half.csv', index=False, header=False)
